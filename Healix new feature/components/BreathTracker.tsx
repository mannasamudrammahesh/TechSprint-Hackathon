...
const BreathTracker: React.FC<BreathTrackerProps> = ({ onBreathUpdate, isActive }) => {
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
        {/* Visual Feedback for Breath Mode */}
        {error ? (
            <div className="bg-red-500/80 text-white px-4 py-2 rounded-full flex items-center gap-2 backdrop-blur-md">
                <AlertCircle className="w-4 h-4" />
                <span className="text-xs font-bold">Mic Error</span>
            </div>
        ) : hasPermission ? (
            <div className="bg-emerald-500/20 border border-emerald-500/50 text-emerald-200 px-6 py-2 rounded-full flex items-center gap-3 backdrop-blur-md animate-pulse">
                <Mic className="w-4 h-4" />
                <span className="text-xs font-bold tracking-widest">LISTENING TO BREATH</span>
            </div>
        ) : (
            <div className="bg-black/50 border border-white/20 text-white/50 px-6 py-2 rounded-full flex items-center gap-2 backdrop-blur-md">
                <MicOff className="w-4 h-4" />
                <span className="text-xs">Initializing Mic...</span>
            </div>
        )}
    </div>
  );
