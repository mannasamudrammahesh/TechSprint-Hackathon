  ...
  return (
    <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10 w-full max-w-2xl px-4">
      <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 shadow-2xl">
        ...
        {/* Top Row: Tension, Audio, Mode */}
        <div className="mb-4 flex items-center space-x-3">
          {/* Mode Switcher */}
          <div className="flex bg-black/40 rounded-full p-1 border border-white/10">
             <button
                onClick={() => onControlModeChange(ControlMode.HAND)}
                className={`p-1.5 rounded-full transition-all ${controlMode === ControlMode.HAND ? 'bg-cyan-500 text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
                title="Hand Control"
             >
             ...
             <button
                ...
                className={`p-1.5 rounded-full transition-all ${controlMode === ControlMode.AUDIO_BREATH ? 'bg-emerald-500 text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
             ...
             <button
                ...
                className={`p-1.5 rounded-full transition-all ${controlMode === ControlMode.VISUAL_BREATH ? 'bg-purple-500 text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
          ...
          {/* Tension Bar */}
          <Activity className={`w-4 h-4 ${tension > 0.8 ? 'text-red-400' : 'text-cyan-400'}`} />
      ...
      <div className="text-center mt-2 text-white/30 text-xs font-light tracking-widest uppercase flex items-center justify-center gap-2">
        {controlMode === ControlMode.HAND && (
            <>
                ...
                <span>Hand/Mouse • Open = Expand • Closed = Contract</span>
            </>
        )}
        {controlMode === ControlMode.AUDIO_BREATH && (
            <>
                <Wind className="w-3 h-3" />
                <span>Mic Breath • Inhale (Silent) = Expand • Exhale (Loud) = Contract</span>
            </>
        )}
        {controlMode === ControlMode.VISUAL_BREATH && (
            <>
                <User className="w-3 h-3" />
                <span>Visual Breath • Shoulders Up (Inhale) = Expand • Shoulders Down = Contract</span>
            </>
        )}
      </div>
    </div>
