...
export interface MusicTrack {
  ...
  description: string;
  benefits: string[];
  keywords: string[]; // For voice recognition
}
...
export class MusicService {
  ...
  async playNext(): Promise<void> {
    try {
      if (!this.currentTrack) {
        // If no track is playing, play a random one
      ...
      if (currentIndex === -1) {
        // Current track not found, play first track
  ...
  async playPrevious(): Promise<void> {
    try {
      ...
      const currentIndex = musicTracks.findIndex(track => track.id === this.currentTrack?.id);
      if (currentIndex === -1) {
        // Current track not found, play last track
...
// Enhanced track database with comprehensive keywords for voice recognition
...
// Utility functions for voice assistant integration
export const getMusicRecommendation = (userInput: string): MusicTrack | null => {
  const input = userInput.toLowerCase();
  
  // Prioritize Healix meditation for peaceful/song requests
  ...
  if (input.includes('stressed') || input.includes('stress')) {
    // First try Healix meditation, then other stress tracks
    const healixTrack = musicTracks.find(t => t.title === 'Healix Meditation');
    if (healixTrack) return healixTrack;
  ...
  // Default to meditation - prioritize Healix track
  const healixTrack = musicTracks.find(t => t.title === 'Healix Meditation');
  if (healixTrack) return healixTrack;
